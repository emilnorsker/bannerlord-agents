...
public static class GameMasterPocQueue
{
	...
	/// <summary>MCM one-shot: synchronous <c>gm.query.kingdom</c> via <c>CommandLineFunctionality</c> — no queue, no OpenRouter, no GM toggles. Writes only to <c>logs/test_log.txt</c>.</summary>
	public static void RunE2ETest()
	{
		const string testLine = "gm.query.kingdom";
		GameMasterE2ETestLog.Append("[BLGM_E2E] === start === path=" + GameMasterE2ETestLog.GetPathForDisplay());
		if (Campaign.Current == null)
		{
			GameMasterE2ETestLog.Append("[BLGM_E2E] FAIL: Campaign.Current is null (load a save / start campaign).");
		...
		GameMasterE2ETestLog.Append("[BLGM_E2E] token=" + command + " args=" + args.Count);
		if (!CommandLineFunctionality.HasFunctionForCommand(command))
		{
			const string msg = "HasFunctionForCommand=false — Bannerlord.GameMaster not loaded or dependency missing in SubModule.xml.";
			GameMasterE2ETestLog.Append("[BLGM_E2E] FAIL: " + msg);
			throw new InvalidOperationException(msg);
		}
		try
		{
			string text = CommandLineFunctionality.CallFunction(command, args, out bool _);
			GameMasterE2ETestLog.Append("[BLGM_E2E] CallFunction OK, len=" + (text?.Length ?? 0));
			GameMasterE2ETestLog.Append("[BLGM_E2E] output:\n" + (text ?? "(null)"));
			GameMasterE2ETestLog.Append("[BLGM_E2E] === success ===");
		}
		catch (Exception ex)
		{
			GameMasterE2ETestLog.Append("[BLGM_E2E] CallFunction threw: " + ex);
			throw;
		}
	}
}
