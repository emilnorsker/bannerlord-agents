...
public static class ToolCatalog
{
	public static JArray GetToolsForApi()
	{
		return new JArray
		{
			...
			Tool("return_to_player", "Return to the player after a task.", "{}", System.Array.Empty<string>()),
			Tool("transfer_troops", "Transfer troops/prisoners. direction: to_player|from_player. transfers: comma list troop:id:count or prisoner:id:count.",
				"{\"direction\":{\"type\":\"string\",\"enum\":[\"to_player\",\"from_player\"]},\"transfers\":{\"type\":\"string\"}}", new[] { "direction", "transfers" }),
			Tool("create_party", "Independent map party. Optional mode outlaw|normal: outlaw forces minor-clan branch when valid; normal forces player-clan lord party.",
				"{\"mode\":{\"type\":\"string\",\"enum\":[\"outlaw\",\"normal\"]}}", System.Array.Empty<string>()),
