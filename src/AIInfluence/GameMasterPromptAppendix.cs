...
public static class GameMasterPromptAppendix
{
	public static string NpcChatBlgmInstructions(Hero npc, NPCContext context)
	{
		...
		sb.AppendLine("{\"gm_command\":\"gm.query.kingdom\",\"args\":[],\"intent\":\"query\",\"probe_help_first\":false}");
	...
	public static string WorldEventsBlgmInstructions(bool queryOnly)
	{
		StringBuilder sb = new StringBuilder();
		sb.AppendLine();
		sb.AppendLine("### OPTIONAL blgm_plan (OpenRouter only) ###");
		sb.AppendLine("You may include \"blgm_plan\": {\"gm_command\":...,\"args\":[...],\"intent\":\"query|mutate|probe_help\",\"probe_help_first\":bool} in the SAME JSON object as your normal fields.");
		if (queryOnly)
		{
	...
	public static string MaybeAppendForBackendRequest(string prompt, string requestType, string backend)
	{
		...
		if (requestType == "dynamic_event_generation" || requestType == "dynamic_events" || requestType == "kingdom_destruction_event")
		{
			...
			return prompt + WorldEventsBlgmInstructions(s.GameMasterWorldEventsBlgmQueryOnly);
		}
		if (requestType == "diplomacy_statement" || requestType == "player_statement_analysis")
		{
			...
			return prompt + WorldEventsBlgmInstructions(s.GameMasterDiplomacyBlgmQueryOnly);
		}
		return prompt;
	}
}
