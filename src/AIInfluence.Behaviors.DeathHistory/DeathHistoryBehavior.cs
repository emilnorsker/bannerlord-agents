...
public class DeathHistoryBehavior : CampaignBehaviorBase
{
	...
	private string RemoveJsonFromText(string text)
	{
		...
		string pattern = "\\{[^{}]*\"(?:response|romance_intent|decision|tone|threat_level|escalation_state|suspected_lie|deescalation_attempt|claimed_name|claimed_clan|claimed_age|claimed_gold|money_transfer|item_transfers|character_personality|character_backstory|kingdom_action|kingdom_action_reason|settlement_id|target_clan_id|daily_tribute_amount|tribute_duration_days|reparations_amount|trade_agreement_duration_years|character_death|workshop_action|workshop_string_id|witnesses|notable_phrases)\":[^{}]*(?:\\{[^{}]*\\}[^{}]*)*\\}";
