...
| 7 | **UI** | Chat VM + `DialogManager` often driven by **one big `AIResponse`** alongside tool side effects | Narrative envelope and gameplay should not duplicate the same facts. |
...
- **Duplicate channels** (envelope fields vs tools) for the same gameplay fact.
...
**Status:** **Started.** Prefer **one tool per map action** with typed args; map behavior is not re-encoded as UI pipe strings.
