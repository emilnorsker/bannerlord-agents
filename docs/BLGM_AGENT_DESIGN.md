...
| **21** | **Observation loop (tool results)** | **Status: done** — `GameMasterObservationStore` + injection via `GameMasterPromptAppendix` (NPC `NpcChat|{id}`, world `World|DynamicEvents`, dip `Dip|Global` / `Dip|PlayerAnalysis`); MCM toggles **Observation loop** / **inject into prompt** / buffer size. |
