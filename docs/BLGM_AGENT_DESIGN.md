...
| **10** | **Diplomacy-scale path (opt-in)** | **Status: done** — MCM **Diplomacy: allow blgm_plan** + **query-only**; prompt appendix in `SendAIRequestWithBackend` for `diplomacy_statement` / `player_statement_analysis`; `blgm_plan` on `DiplomaticStatementResponse`, `PlayerStatementAnalysisResponse`. **Dynamic event generation does not use BLGM** (events system stays separate). |
| **11** | **Hazard index v1** | **Superseded by slice 23 behavior in code** — `gm.query.*` passes early; `gm.kingdom.*` ruler/kingdom checks; high-risk families (`gm.troop`/`bandit`/`caravan`) blocked when enforcement on; NPC prisoner blocks high-risk with interlocutor. MCM **Enforce hazard preconditions**. |
| **12** | **Full tagged command index** | **Status: partial** — appendix lists families + example queries; **generated ~139 table** still future work. |
...
| **16** | **OpenRouter-only for GM plans** | **Status: done** — `GameMasterPlanExecutor` ignores `blgm_plan` unless backend name is **OpenRouter** (main AI for NPC Chat; Diplomacy AI for diplomacy completions). |
| **17** | **Schema-constrained plans** | **Status: partial** — POC strict schema includes `story_intent`; full dialogue envelope unchanged. See **“When to use json_schema vs json_object”** below. |
| **18** | **RAG (optional)** | **Status: stub** — `GameMasterWikiRag.TryRetrieve` returns empty until a retrieval backend exists. |
| **19** | **Campaign lifecycle** | **Status: partial** — `GameMasterLifecycle.OnGameEnd` drains GM queue, clears observations + host policy counters; **no** save-game persistence or mission-boundary policy yet. |
| **20** | **MissionGM** | **Status: placeholder** — `MissionGmScope` documents separation; mission-time operator **not** implemented. |
| **21** | **Observation loop (tool results)** | **Status: done** — `GameMasterObservationStore` + injection via `GameMasterPromptAppendix` (NPC `NpcChat|{id}`, dip `Dip|Global` / `Dip|PlayerAnalysis`); MCM toggles **Observation loop** / **inject into prompt** / buffer size. |
| **22** | **Host allowlist policy (layer 3)** | **Status: done** — `GameMasterHostPolicy`: `gm.` prefix, max line length (default 1024), per-campaign-day cap, sliding rate window + max plans/window, queue depth (with extra slot for `probe_help_first`); `[GM_POLICY]` logs. |
| **23** | **Hazard index v2** | **Status: done (pragmatic v2)** — `gm.query.*` allowed first; kingdom/troop/bandit/caravan rules as in slice 11 row; **interlocutor** used for prisoner + high-risk combo. Per-command wiki codegen still future. |
| **24** | **NPC GM appendix v2 (social stats)** | **Status: done** — MCM **NPC Chat: append relation line**; `GetRelation` with player; **not** appended for world appendix. |
| **25** | **Skill-check gates for mutates (optional)** | **Status: done (opt-in)** — MCM **Skill check before risky mutates**; opposed **social** when NPC hero present and intent is mutate (non-query). |
| **26** | **No UI disambiguation (real implementation)** | **Status: future** — engine hook TBD. |
| **27** | **Generated OpenRouter grammars (per GM command / family)** | **Status: partial** — `schemas/gm/README.md` + in-code `BuildOpenRouterGmPlanResponseFormat`; **CI/codegen** for per-family files not wired. |
| **28** | **`story_intent` in plan schema + pipeline** | **Status: done** — `BlgmPlanDto.story_intent`, strict schema field, audit + observation rows carry it. |
