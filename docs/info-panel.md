...
| 5 | World events      | Recent dynamic events (`GlyphLines`): icon from `DynamicEvent.Type`; row underlay = Native `scrollable_field_gradient_9` with `Color` from `Importance` (1–10). Section header icon: Scouting. “None” uses `TextLines`. |
...
| VM | Gauntlet |
|----|----------|
| `InfoSectionVM.GlyphLines` (`InfoGlyphLineVM`) | **World events only:** optional `HasWorldEventBackground` + `SeverityTintColor` + `scrollable_field_gradient_9`; `IsSkill` + `SkillIconVisualWidget` + text. |
