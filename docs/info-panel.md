...
| 5 | World events      | Recent dynamic events (`GlyphLines`): icon from `DynamicEvent.Type`; row underlay = Native `scrollable_field_gradient_9` behind text via `DimensionSyncWidget` (~10% `AlphaFactor`, no color multiply). Text `Color` gets a slight nudge from `Importance` via `WorldEventGlyphHelper.GetWorldEventTextColor`. Section header icon: Scouting. “None” uses `TextLines`. |
...
| VM | Gauntlet |
|----|----------|
| `InfoSectionVM.GlyphLines` (`InfoGlyphLineVM`) | **World events only:** optional `HasWorldEventBackground` + untinted `scrollable_field_gradient_9` (sized to `RichTextWidget` via `DimensionSyncWidget`); `Color` from `GetWorldEventTextColor(importance)`; `IsSkill` + `SkillIconVisualWidget` + text. |
...
Prefab: `GUI/Prefabs/ChatInterface.xml` — **`ChatMainInset`**: **`Frame1Brush`** + low-alpha **`frame_9`**. Full-width **hero strip** (name, occupation `#E1BE8FFF`, traits) + **`Encyclopedia\list_filters_divider`**; then three columns. **`InfoScrollPanel`** margins **`28` / `78` / `74`** (= 4+24 sides, 54+24 top, 50+24 bottom); no extra wrapper. Section header row: **`Encyclopedia.SubPage.Info.Text`** **`Brush.FontColor="#E1BE8FFF"`**, **17px** (occupation styling; not title size). Inner lists use **0** lateral margin. Section headers use **`SkillIconVisualWidget`** + `InfoSectionVM.HeaderSkillId` (vanilla skill art). Expand/collapse uses ASCII **`v` / `>`** (Unicode arrows were missing glyphs and showed “?”). **`NavigationScopeTargeter`** (`NpcChatInfoSectionsScope`) → **`InfoSectionsList`**. **`InfoScrollPanel`** (`ClipRect` = **`InfoClipRect`**, inner = **`InfoSectionsList`**); sibling **`InfoScrollBar`** (invisible). Expanded body order: **GlyphLines → TextLines → `NpcPartyTroopCountsParent` (party section only) → party food**.
